...
/**
 * Sign the previousEpochsSpentAddresses file with a key built with {@link KeyfileBuilder}.
 * 
 * (c) 2017 Michael Schierl. Licensed under MIT License.
 */
public class AddressSigner {
	...
	public static void main(String[] args) throws Exception {
		if (args.length < 2) {
			System.out.println("Usage: addressSigner <keyfile> <keyIndex>");
			return;
		}
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream("previousEpochsSpentAddresses.txt")))) {
		...
		kerl.squeeze(trits);
		System.out.println("Address hash: " + Converter.trytes(trits));
		System.out.println("Public subkey: " + keyfile[0][keyIndex]);
		int[] bundle = new Bundle().normalizedBundle(Converter.trytes(trits));
		try (BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("previousEpochsSpentAddresses.sig")))) {
		...
		System.out.println("previousEpochsSpentAddresses.sig created.");
	}
}
